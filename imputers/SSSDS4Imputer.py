...
class SSSDS4Imputer(nn.Module):
    ...
    def forward(self, input_data):
        ...
        #conditional = self.enncoder(conditional) # take Nconditions x 50 and squase to size of noise(144 x 50) for any nunber of conditions which are temporal i.e. weather

        #conditional = self.encoder(conditional) # take Ncondistions x 50 which are not temporal (ie. numbers of customers) and squase to 1 x 50
        # then repmat rows to make it 144x50
